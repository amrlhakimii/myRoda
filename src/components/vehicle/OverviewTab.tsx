...
export function OverviewTab({ vehicle }: { vehicle: Vehicle }) {
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <Card className="p-5">
        <h3 className="text-sm font-bold text-mist-100">Vehicle details</h3>
        <dl className="mt-3 grid grid-cols-2 gap-4 text-sm sm:grid-cols-3">
          <div>
            <dt className="text-mist-500">Brand</dt>
            <dd className="font-medium text-mist-100">{vehicle.brand}</dd>
          </div>
          <div>
            <dt className="text-mist-500">Model</dt>
            <dd className="font-medium text-mist-100">{vehicle.model}</dd>
          </div>
          <div>
            <dt className="text-mist-500">Year</dt>
            <dd className="font-medium text-mist-100">{vehicle.year}</dd>
          </div>
          <div>
            <dt className="text-mist-500">Registration</dt>
            <dd className="font-medium text-mist-100">{vehicle.registrationNumber}</dd>
          </div>
        </dl>
      </Card>
      ...
      <Card className="p-5">
        ...
        {upcomingReminders.length === 0 ? (
          <p className="mt-2 text-sm text-mist-500">No reminders set for this vehicle yet.</p>
        ) : (
              ...
              return (
                ...
                >
                  <div>
                    ...
                    <p className="text-xs text-mist-500">{formatDate(reminder.nextDueDate)}</p>
                  </div>
