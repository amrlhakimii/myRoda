...
export function RecentActivity({ latestService, latestFuel, vehicleNameById }: RecentActivityProps) {
  return (
    <Card className="p-5">
      <h3 className="text-sm font-bold text-mist-100">Recent activity</h3>
      <div className="mt-3 flex flex-col gap-2">
        {latestService && (
          <Link
            to={`/vehicles/${latestService.vehicleId}`}
            className="flex items-center gap-3 rounded-xl bg-white/5 px-3.5 py-3 hover:bg-white/10"
          >
            <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-steel-100 text-steel-700">
              <Wrench size={16} />
            </div>
            <div className="min-w-0 flex-1">
              <p className="truncate text-sm font-semibold text-mist-100">{latestService.workshopName}</p>
              <p className="text-xs text-mist-500">
                {vehicleNameById.get(latestService.vehicleId)} · {formatDate(latestService.date)}
              </p>
            </div>
            <span className="text-sm font-bold text-mist-100">{formatCurrency(latestService.totalCost)}</span>
          </Link>
        )}
        {latestFuel && (
          <Link
          ...
          >
            ...
            <div className="min-w-0 flex-1">
              <p className="truncate text-sm font-semibold text-mist-100">{latestFuel.petrolType} refuel</p>
              <p className="text-xs text-mist-500">
                {vehicleNameById.get(latestFuel.vehicleId)} · {formatDate(latestFuel.date)}
              </p>
            </div>
            <span className="text-sm font-bold text-mist-100">{formatCurrency(latestFuel.totalPrice)}</span>
          </Link>
        )}
        {!latestService && !latestFuel && (
          <p className="text-sm text-mist-500">No activity yet. Add a service or fuel record to see it here.</p>
        )}
      </div>
    </Card>
  )
